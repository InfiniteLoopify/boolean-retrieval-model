...
class Table:
    ...
    def display_Table(self, answer_list):
        ...
        for i, doc_id in enumerate(answer_list):
            file_name = "speech_" + str(doc_id) + ".txt"
            f = open("speech/" + file_name, "r")
            title = f.readline()
            self.listBox.insert("", "end", values=(i + 1, file_name, title))
    ...
    def create_Gui(self, indexer):
        ...
        answer = tk.StringVar()
        searchQuery = tk.Entry(self.scores, width=70, textvariable=answer).grid(
            row=1, column=0
        )
        ...
        self.listBox.column(self.cols[1], minwidth=50, width=150, stretch=tk.NO)
        ...
        showScores = tk.Button(
            ...
            command=lambda: [
                ...
                self.display_Table(indexer.search_query(answer.get())),
            ],
        ).grid(row=1, column=1)
