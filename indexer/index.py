...
class Indexer:
    ...
    def read_file(self, path):
        ...
        for doc_id, file_name in enumerate(file_list):
            ...
            with open(path + file_name, 'r') as file_data:
                token_file.write("\nDOC-" + str(doc_id) + '_______\n')
                file_data.readline()  # ignore first line of every file
    ...
    def index_to_file(self, path):
        i_index_file = open(path+'i_index.txt', 'w')
        p_index_file = open(path+'p_index.txt', 'w')
        for elem in self.i_index:
            i_index_file.write(elem + ':\n' + str(self.i_index[elem]) + '\n\n')
        for elem in self.p_index:
            p_index_file.write('\n' + elem + ': \n')
            for doc in self.p_index[elem]:
                p_index_file.write(str(doc) + ': ' +
                                   str(self.p_index[elem][doc]) + '\n')

    # insert token to inverted index
    def inverted(self, word, doc_id):
        if word not in self.i_index:  # if word not present then add new entry to dict
            self.i_index[word] = []
            self.i_index[word].append(doc_id)
        elif doc_id not in self.i_index[word]:
            self.i_index[word].append(doc_id)

    # insert token to positional index
    def positional(self, word, word_pos, doc_id):
        if word in self.p_index:  # if word present then append to dict
            if not self.p_index[word].get(doc_id):
                self.p_index[word][doc_id] = []
            self.p_index[word][doc_id].append(word_pos)
    ...
    def proximity_query(self, q1, q2, space):
        ...
        if self.p_index.get(q1) != None and self.p_index.get(q2) != None:
            for doc in self.p_index[q1]:
                if doc in self.p_index[q2]:
                    for pos in self.p_index[q1][doc]:
                        if pos + space in self.p_index[q2][doc]:
                            if doc not in ans:
                                ans.append(doc)
        return ans
    ...
    def evaluate_postfix(self, expression):
        ...
        for term in expression:
            ...
            # if not operator then keep apending to stack
                ...
                ans.append(self.i_index.get(temp_str, []))
        return ans.pop()
...
if __name__ == "__main__":
    ...
    print(query_ans)
    print(len(indexer.i_index))
    print(len(indexer.p_index))
